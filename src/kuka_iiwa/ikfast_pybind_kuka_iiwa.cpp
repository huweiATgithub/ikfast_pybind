...
PYBIND11_MODULE(ikfast_kuka_iiwa, m)
{
    m.doc() = R"pbdoc(
        kuka_iiwa
        -----------------------
        .. currentmodule:: kuka_iiwa
        .. autosummary::
           :toctree: _generate
           get_ik
           get_fk
    ...
    m.def("get_ik", [](const std::vector<double> &trans_list,
          ...
          py::arg("free_jt_vals"),
          R"pbdoc(
        get inverse kinematic solutions for kuka_iiwa
    )pbdoc");
    ...
    m.def("get_fk", [](const std::vector<double> &joint_list)
          ...
          py::arg("joint_list"),
          R"pbdoc(
        get forward kinematic solutions for the kuka_iiwa robot
    )pbdoc");
